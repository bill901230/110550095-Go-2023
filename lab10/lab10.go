...
var swear_words []string
var sensitive_names []string
...
func InitObservable() {
	...
	*/
	ObservableMsg = ObservableMsg.Filter(func(i interface{}) bool {
		...
		msg := i.(string)
		for _, s := range sensitive_names {
			index := strings.Index(msg, s)
			if index != -1 {
				return msg[:index+3] + "*" + msg[index+6:], nil
...
func main() {
	...
	for scanner.Scan() {
		swear_words = append(swear_words, scanner.Text())
	}
	file, err1 := os.Open("sensitive_name.txt")
	if err1 != nil {
		return
	}
	defer file.Close()
	...
	for scanner1.Scan() {
		sensitive_names = append(sensitive_names, scanner1.Text())
	}
	InitObservable()
	go broadcaster()
